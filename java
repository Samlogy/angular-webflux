...
* logging
* monitoring + graph
* monitoring advanced + graphs: rabbit, posgres, redis, ...
...
curl -X POST "http://localhost:8080/realms/myrealm/protocol/openid-connect/token" \
    ...
    -d "grant_type=password&client_id=springboot-app&client_secret=your-client-secret&username=testuser&password=password"



Access Public Endpoint (No Token Needed)
curl -X GET "http://localhost:4003/api/orders/public"

Access User Endpoint
curl -X GET "http://localhost:8081/api/orders/user" -H "Authorization: Bearer YOUR_ACCESS_TOKEN"

Access Admin Endpoint (Fails for testuser)
curl -X GET "http://localhost:8081/api/orders/admin" -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
...
Manages build artifacts, dependencies, and Docker images.
...
docker exec -it 18edcc5e2741 cat /nexus-data/admin.password

# access next locally 
## singin: admin/ "password"
http://localhost:8081
```

Il y a un 3 principales types de repository:
- Hosted Repository : pour publier tes jars de ton app Spring.
- Proxy Repository : pour cacher Maven Central (éviter de pomper internet à chaque build).
- Group Repository : pour simplifier (un seul URL d'accès).

**Hosted Repo:**
Repositories > Create repository > maven2 (hosted)
Name : maven-releases
Version policy : Release
Write policy : Allow redeploy (pour test/dev ; en prod => "disable redeploy")

**Proxy Repo:**
Repositories > Create repository > maven2 (proxy)
Name : maven-central
Remote URL : https://repo.maven.apache.org/maven2/
Laisse le reste par défaut, crée.

**Group:**
Repositories > Create repository > maven2 (group)
Name : maven-public
Member repositories : sélectionne maven-releases, maven-snapshots, maven-central



**NB:**
En production:
- faire persister les données sur un volume durable.
- access https: reverse procy (nginx) + ssl certificate.
- installtion sur vm (local install).
- 
